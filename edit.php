...
/////////////////////////////////////////////////////////////////////////////
/**
 * Gathers and updates a record into a panel.
 * 
 * @package Dacomasy
 * @subpackage WebClient
 * @version $SixXGate: webapps/dacomasy/www/edit.php,v 1.1.1.1 2005/07/21 22:57:31 matthew Exp $
...
<?php
	...
	case 'okay':
		...
		if (DacomasyValidateForm($module, $PanelIndex, $Values, $Messages, TRUE) === FALSE) {
			...
			//--></script>
<?php		echo DacomasyGenerateForm($module, $PanelIndex, $ModuleFilename, $Values, $Messages, TRUE);
		} else {
			DacomasyPrepareValuesArray($module, $PanelIndex, $Values);
			if (DacomasyUpdateRecord($module, $PanelIndex, $Values) === FALSE) {
				echo '<p>' . $__dacomasy__MSG_res[__DACOMASY_MSG_EDITFAILED] . '</p>';	// TODO: make update message
				echo '<p>' . DacomasyGetLastErrorMsg() . '</p>';
			} else {
				echo '<p>' . $__dacomasy__MSG_res[__DACOMASY_MSG_EDITSUCCESS] . '</p>';	// TODO: make update message
			}?>
			<script><!--
				parent.frames.DacomasyTitle.location = 'title.php?title=<?php echo urlencode($module['Panels'][$PanelIndex]['Name'] . ' - ' . $__dacomasy__LABEL_res[__DACOMASY_LABEL_ADDRECORD]); ?>&<?php echo DacomasySID(); ?>'
...
<?php		// TODO: Create and use the add record code to add the record...
	...
	default:?>
		<script><!--
			...
			parent.frames.DacomasyFunctions.location = 'functions.php?mode=form&<?php echo DacomasySID(); ?>';
		//--></script>
<?php	$Values = DacomasyGetRecord($module, $PanelIndex, $RecordIndex);
		DacomasyUNPrepareValuesArray($module, $PanelIndex, $Values);
		
		echo DacomasyGenerateForm($module, $PanelIndex, $ModuleFilename, $Values, $GLOBALS['ByRefNull'], TRUE);
	} // DACO_action
?>
</body>
</html>
